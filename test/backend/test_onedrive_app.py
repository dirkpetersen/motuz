...
from api.utils.abstract_connection import subprocess_env
...
class TestSubprocessEnv(unittest.TestCase):
    ...
    def test_server_secrets_removed(self):
        ...
        with mock.patch.dict(os.environ, {**secrets, 'MOTUZ_ONEDRIVE_CLIENT_ID': 'id', 'PATH': '/bin'}):
            env = subprocess_env({'RCLONE_CONFIG_SRC_TYPE': 'onedrive'})
        for key in secrets:
            self.assertNotIn(key, env)
        self.assertEqual(env['MOTUZ_ONEDRIVE_CLIENT_ID'], 'id')
